...
{
	internal class TypeCorrector : SqlVisitor
	{
...
#warning [FB] REFACTOR SQL Server specific. 
			...
			if(sql.IsDateType(arg1) && !sql.IsHighPrecisionDateTimeType(arg2))
			{
				SetSqlTypeIfSimpleExpression(arg2, arg1.SqlType);
			}
			else if(sql.IsDateType(arg2) && !sql.IsHighPrecisionDateTimeType(arg1))
			{
