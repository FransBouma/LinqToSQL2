...
namespace System.Data.Linq.Provider.Visitors
{
	...
	/// </summary>
	internal class ColumnLifter : SqlVisitor
	{
		...
		private class SelectScope
		{
			...
			private Stack<List<SqlColumn>> _lifted = new Stack<List<SqlColumn>>();
			internal IEnumerable<SqlAlias> LeftProduction { get; set; }
			private HashSet<SqlColumn> _referencedExpressions = new HashSet<SqlColumn>();

			internal Stack<List<SqlColumn>> Lifted
			{
				get { return _lifted; }
			}

			internal HashSet<SqlColumn> ReferencedExpressions
			{
				get { return _referencedExpressions; }
			}
		}
		#endregion
