...
namespace System.Data.Linq
{
	...
	/// class which is used as a hub, the actual services are performed by objects controlled by an instance of this object.
	/// </summary>
	internal class CommonDataServices : IDataServices
	{
		...
		private IdentityManager _identifier;
		private ChangeTracker _tracker;
		private ChangeDirector _director;
		private bool _hasCachedObjects;
		private Dictionary<MetaDataMember, IDeferredSourceFactory> _factoryMap;
		#endregion
		...
		private class DeferredSourceFactory<T> : IDeferredSourceFactory
		{
			...
			#region Private classes
			/// <summary>
			/// [FB] Nested class in a nested class... InceptionDeferredSource? ;)
			/// Left here as it's just a glorified enumerator.
			/// </summary>
...
#pragma warning restore 618
 mm.Type
		...
		internal CommonDataServices(DataContext context, MetaModel model)
		{
			...
			this._identifier = IdentityManager.CreateIdentityManager(asReadOnly);
			this._tracker = ChangeTracker.CreateChangeTracker(this, asReadOnly);
			this._director = ChangeDirector.CreateChangeDirector(context);
			this._factoryMap = new Dictionary<MetaDataMember, IDeferredSourceFactory>();
		}
		...
		{
			_hasCachedObjects = false;
			bool asReadOnly = !_context.ObjectTrackingEnabled;
			this._identifier = IdentityManager.CreateIdentityManager(asReadOnly);
			this._tracker = ChangeTracker.CreateChangeTracker(this, asReadOnly);
			this._factoryMap = new Dictionary<MetaDataMember, IDeferredSourceFactory>();
		}
		...
		internal object GetCachedObject(MetaType type, object[] keyValues)
		{
			...
			return this._identifier.Find(type, keyValues);
		}
		...
		internal object GetCachedObjectLike(MetaType type, object instance)
		{
			...
			return this._identifier.FindLike(type, instance);
		}
		...
		public bool IsCachedObject(MetaType type, object instance)
		{
			...
			return this._identifier.FindLike(type, instance) == instance;
		}
		...
		public object InsertLookupCachedObject(MetaType type, object instance)
		{
			...
			_hasCachedObjects = true;  // flag that we have cached objects
			if(!type.IsEntity)
			{
				return instance;
			}
			return this._identifier.InsertLookup(type, instance);
		}
		...
		public bool RemoveCachedObjectLike(MetaType type, object instance)
		{
			...
			return this._identifier.RemoveLike(type, instance);
		}
		...
		public void OnEntityMaterialized(MetaType type, object instance)
		{
			...
			this._tracker.FastTrack(instance);
		...
		{
			ITable table = this._context.GetTable(type.InheritanceRoot.Type);
		...
		internal Expression GetDataMemberQuery(MetaDataMember member, Expression[] keyValues)
		{
			...
			if(member.IsAssociation)
			{
				...
				Expression thisSource = Expression.Constant(_context.GetTable(rootType));
		...
		public IDeferredSourceFactory GetDeferredSourceFactory(MetaDataMember member)
		{
			...
			if(this._factoryMap.TryGetValue(member, out factory))
			{
				return factory;
			}
			...
			this._factoryMap.Add(member, factory);
			return factory;
		}
		...
		internal object GetObjectByKey(MetaType type, object[] keyValues)
		{
			...
			if(target == null)
			{
				...
				target = ((IEnumerable)this._context.Provider.Execute(this.GetObjectQuery(type, keyValues)).ReturnValue).OfType<object>().SingleOrDefault();
			}
			return target;
		}
	}
